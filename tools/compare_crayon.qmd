...
Manually copy the shapes.geojson and stops.geojson files used to generate the GTFS being assessed to the /dep/results_log/\<crayon\> folders before running this script.

## Select the Crayon

Choose the crayon to compare in /dep/results_log/\<crayon\>

```{r}
crayon <- "expo_hastings_ext" #"crayon" # Folder with crayon in ../dep/results_log

# These should come from an attribute of the lines in shapes.geojson
...
route <- st_read(paste(results_folder, crayon, "/shapes.geojson", sep = ""))
stops <- st_read(paste(results_folder, crayon, "/stops.geojson", sep = ""))

new_routes <- st_read(paste(results_folder, "/proposed/shapes.geojson", sep = ""))
new_stops <- st_read(paste(results_folder,"/proposed/stops.geojson", sep = ""))

more_routes <- st_read(paste(results_folder, crayon, "/shapes.geojson", sep = ""))
more_stops <- st_read(paste(results_folder, crayon, "/stops.geojson", sep = ""))

# Existing network
file1 <- paste(results_folder, "existing/transit_job_accessibility.csv", sep = "")
file2 <- paste(results_folder, crayon, "/transit_job_accessibility.csv", sep = "")
...
```{r}
ggsave(
  paste(results_folder, crayon, "/transit_job_access_change_plot.png", sep = ""),
  units = "in", 
  width = 10, 
  height = 5.8, 
  dpi = 400
...
write.csv(access_sf, paste(results_folder, crayon, "/transit_job_access_change.csv", sep = ""), row.names=FALSE)
```
